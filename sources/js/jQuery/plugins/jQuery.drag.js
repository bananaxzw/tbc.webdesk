	...
	$.fn.drag = function(settings){
		...
		var _updateArea = function()
		{
			...
			if(a===true)
			{
				...
				_area = {left: a.left, top:a.top, right:a.right, bottom:a.bottom};
			}
			
			var m=options.areaMargin,
				c=tbc.percentToInt;
			if(m && m.left && m.top){
		...
		return this.each ( function ( index, _this ) {
			...
			var timeout; // 鼠标按下后延迟一定时间再触发拖放动作
			...
			var handle = $(options.handle, _this);
			...
			var   mouseStartPos
				, mouseToTargetTop
				, mouseToTargetLeft;
			var _scrTop  = options.document.documentElement.scrollTop ||options.document.body.scrollTop,
			...
			handle.bind({
				...
				"touchstart" : function( event ){
					...
					$(this).bind({
						"touchmove.touchDrag": function(event){
							...
							_this.css({left: x - _touchstartOffset.x, top:y - _touchstartOffset.y});
							return false;
						},
				...
				"mousedown"  : function(event){
					...
					function mouseup_drag(event){
						...
						var dropon = _ghost ? _ghost[0] : null;
						event.dropon = dropon;
						var insertable = $.isFunction(options.event.drop) 
						...
						if( options.target )
						{
							var eTarget = _ghost.parent();
							$.extend(event, {target:eTarget});
							
							var xy = tbc.getCoordinate( _ghost[0] );
			...
			function getDragingOffset(event)
			{
				...
				var offset = tbc.getCoordinate(_wrap[0].offsetParent);
				var parentOff = tbc.getCoordinate(_wrap[0].offsetParent);
				...
				if(options.area)
				{
					o.top = o.height >= _area.height
						? o.top
						: o.bottom >= _area.bottom
							? _area.bottom - o.height
							: Math.max(_area.top, o.top);
					o.left = o.width >= _area.width
						? o.left
						: o.right >= _area.right
							? _area.right - o.width
							: Math.max(_area.left, o.left);
				}
