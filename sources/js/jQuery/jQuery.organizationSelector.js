(function($){
             ...
             * 批量方法
             */
            $.batch = $.batch || function ( queue, size, current, timeout, method, callback, _nextPort ){
                ...
                if ($.isFunction(method)) {
                    ...
                    console.log("Error: $.batch(...). arguments[4] must be a function or a method of tbc.Tree();");
                }
             ...
             * 综合选择组件
             */
            $.fn.organizationSelector = function( settings ) {
                ...
                var id = "organizationSelector_" + new Date().getTime(),
                    zIndex = window.tbc ? tbc.getMaxZIndex() : 10,
                    html = $('<div id="'+ id +'"/>').css({ width:options.width||780, height:options.height||480 }),
                    iframe = $('<iframe name="' + id + '_iframe" src="about:blank" frameborder="0" hidefocus="true" scrolling="no" style="position:relative; width:100%; height:100%; " allowTransparency="true"></iframe>').appendTo(html),
                    height = Math.max(
                ...
                if ( this.length!==0 ) {
                    ...
                    html.css({
                        position : "absolute", zIndex:zIndex, left:"50%", top:"50%", marginLeft:0-((options.width||740)/2), marginTop:0-((options.height||480)/2)
                    })
                    .appendTo("body").show()
                    .addClass("PIE");
                }
                ...
                for ( var kk in options.post ) {
                    form.append('<input name="'+ kk +'" value="'+ options.post[kk] +'" type="hidden" />');
                }
