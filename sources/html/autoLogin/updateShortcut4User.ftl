            ...
            <div class="scdialog_main">
                <table class="scform" style="font-size:12px">
                    <tr>
                        ...
                        <td>
                            <div class="scform_cell"><input required length="10" type="text" name="appName" pattern="appName"
                                                            value="${appName!}"
                                                            label="应用名称"/><em
...
<script type="text/javascript">
    ...
    function removeShortcut() {
        $.tbcmsg({
            ...
            onOk:function () {
                $.ajax({
                    url:'elos.removeUserShortcut.do?deskItemId=${deskItemId!}',
                    type:'POST',
                    success:function () {
                        var li = $('body').find(".prgico .app[data-appid='${deskItemId!}']")
                        if (li.size() > 0) {
                            li.remove();
    ...
    function submitForm() {
        if (validateForm('updateShortcutForm')) {
            ...
            $.ajax({
                ...
                async:false,
                success:function (d) {
                    var appData = $.extend(d.userDeskItem, {
                        itemType:'ICON',
                    ...
                    var itemObj = $("div.desktop").first().find("li[data-appid='${deskItemId!}']");
                    itemObj.data('appData', appData);
                    itemObj.find('a').attr('title',dataObj.appName).attr('href',dataObj.accessUrl);
