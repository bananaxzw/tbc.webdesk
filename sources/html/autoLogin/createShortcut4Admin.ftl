...
<div class="mscmain">
    ...
    <form id="addShortcut4AdminForm">
        <div class="scdialog_panel">
            <div class="scdialog_main">
                <table class="scform" style="font-size:12px">
                    ...
                    <tr>
                        <td width="80" align="right" valign="top" style="color:#ABABAB"><i id="accessUrlItem" style="display: none;">*</i>登录地址:</td>
                        <td>
                            <div class="scform_cell"><input pattern="url" label="登录地址" name="accessUrl"
                                                            type="text"/><em
                                    class="sctip_right hide"></em></div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="scform_cell"><label><input type="checkbox" name="showInUserDesk" value="true"
                                    /><b>是否显示到学员桌面</b></label><span
                    ...
                    <tr id="appCodeRow">
                        ...
                        <td>
                            <div class="scform_cell"><input required remote pattern="word" length="20" label="应用编号"
                                                            name="appCode"
                                                            type="text"/><em
                                    class="sctip_right hide"></em></div>
                            <div class="scform_cell"><span class="sctip_normal">应用编号长度请控制在20字以内</span></div>
                        </td>
                    </tr>
                </table>
            </div>
        ...
        <input type="hidden" name="needSet" value="false">

        <div class="sctit_tips mstit_cell" style="padding:5px 8px;">
            <div class="mstit_cell_left" style="line-height:21px;"><b>自动登录</b></div>
            <div class="mstit_cell_right" id="msLgSwitch"><span class="msSwitch">启用</span><span
                    class="msSwitch msSwitchCur">禁用</span></div>
        </div>
...
<script type="text/javascript">
    $(function () {
        initValidate('addShortcut4AdminForm');
    <#--修改图片-->
            ...
            switch (idx) {
                case 0:
                    if ($('#msMainLoader').html() == '') {
                        ...
                        $('#msMainLoader').load('elos.toShortcutSet.do');
    ...
    function submitForm() {
        if (validateForm('addShortcut4AdminForm')) {
            var data = $('#addShortcut4AdminForm').serializeArray();
            ...
            $.ajax({
                ...
                success:function ( d ) {
                    var info = '操作成功!';
					/** Luozhihua **
                    if (dataObj.showInUserDesk != undefined) {
                        info += '刷新后,快捷方式将会显示在第一屏。'
                    }
					**/
					...
					if ( dataObj.showInUserDesk != undefined ) {
						...
						var type,
							desk;
						for ( var i=0,len=desktop.tabs; i<len; i++ ) {
							type = desktop.tabs[i].options.type;
							if ( type!="page" && type!="admin" ) {
								desk = desktop.tabs[i];
								
								// 如果桌面存在:将快捷方式显示到桌面
								if ( desk ) {
									desk.appenShortcuts( d.userDeskItem );
								}
								continue;
							}
						}
						...
						var desk = desktop.current(),
							type = desk.options.type;
